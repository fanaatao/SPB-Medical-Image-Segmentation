...
def level_set(I, init_mask, max_its=200, alpha=0.2,
...
def post_process(img):
    ...
    mask[mask < 255] = 0
    mask = mask / 255
    seg, _, _ = level_set(img, mask, max_its=800, display=False, alpha=1.0)
    return seg*255


if __name__ == "__main__":
    img = imread('example.png', flatten=True)
    # mask = np.zeros(img.shape)
    # mask[20:100, 20:100] = 1

    seg = post_process(img)

    # seg, _, _ = level_set(img, mask, max_its=500, display=True, alpha=1.0)